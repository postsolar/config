...

{
  ...
  programs.fish = {
    ...
    shellInit = # fish
      ''
      # All of the below would be required to properly set up nix-related fish shell init on a system where fish
      # is not managed by nix-darwin, as built-in fish init files that come with nix (but not nix-darwin) are deficient.
      # Commented out while I'm setting up nix-darwin.
